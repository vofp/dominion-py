...
class Card:
	...
	def cutpurse(self,p,ch1, ch2, ch3):
		p.coins += 2
		for player in p.gameState.players:
			if player != p:
				discardCard(CardName.copper)

	def embargo(self,p,ch1, ch2, ch3):
		p.coins += 2
		ch1.embargoTokens += 1
	...
	def mine(self,p,ch1, ch2, ch3):
		...
		if cards[ch2].type != 1:
			return -1
		if cards[c].cost + 3 > cards[s].cost:
			return -1
	...
	def great_hall(self,p,ch1, ch2, ch3):
		p.numActions += 1
		p.drawCard()
...
cards[CardName.ambassador] = Card(CardName.ambassador,Card.ambassador,9,9)
cards[CardName.baron] = Card(CardName.baron,Card.baron,9,9)
cards[CardName.council_room] = Card(CardName.council_room,Card.council_room,2,5)
cards[CardName.cutpurse] = Card(CardName.cutpurse,Card.cutpurse,9,9)
cards[CardName.embargo] = Card(CardName.embargo,Card.embargo,9,9)
cards[CardName.feast] = Card(CardName.feast,Card.feast,2,4)
cards[CardName.gardens] = Card(CardName.gardens,Card.defaultAction,9,9)
cards[CardName.great_hall] = Card(CardName.great_hall,Card.great_hall,9,9)
...
class GameState:
  ...
  #   }
	...
	def isGameOver(self):
		if cards[CardName.province].count == 0:
			return 1
		count = 0
		for c in cards:
			if c.count == 0:
...
class Player:
	...
	def playCard(self,c, ch1, ch2, ch3, g):
		...
		r = cards[card].action(self, self, ch1, ch2, ch3, c)
